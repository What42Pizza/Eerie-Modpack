...
float getHorizonMultiplier() {
		...
		altitudeAddend -= (atan(eyeAltitude-64)+PI/2)/5; // shrink horizon faster when you're high off the ground
...
vec3 getSkyColor() {
		...
		return mix(dstrt(skyColor, 0.05), dstrt(fogColor, 0.01), fogAmount);
	#elif defined NETHER
